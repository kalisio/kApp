...
rclone copy android.build.log aws:${BUILD_BUCKET}/android.build.log
check_code $EXIT_CODE "Building the app"

# Backup the android build to S3
rclone copy src-cordova/platforms/android/app/build/outputs/apk aws:${BUILD_BUCKET}/android > /dev/null
...
EXIT_CODE=$?
# Copy the log whatever the result
rclone copy android.deploy.log aws:${BUILD_BUCKET}/android.deploy.log
